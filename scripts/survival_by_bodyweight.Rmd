...
Read in dataset
```{r}
survival <- read_csv("survival_after_acclimation.csv")
survival_original <- read_csv("survival_original.csv")
slug_weights <- read.csv("NMR_data//FINAL_NMR_BAYESIL_processed//slug_weights.csv")
...
Look at survival proportions for all slugs
Standard Error of the proportion (where p is the sample proportion) = √p̂(1-p̂) / n
```{r}
...
lineplot.CI(acclimation_temp,averageSCP,acclimation_day_length,data=survival_stats, 
            lwd = 5,
            cex = 2,
...
weight.anova <- aov(weight~acclimation_day_length*acclimation_temp,data=survival_stats)
...
lineplot.CI(acclimation_temp,averageSCP,acclimation_day_length,
            ...
            x.leg = 1.8,
            cex.axis = 1.2) 
#paste this into command line to be able to ajust the plot in quartz
